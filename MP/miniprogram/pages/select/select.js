...
Page({
   ...
   */
  data: {
    problemset:['请选择题目','1. Invent Yourself 自己发明','2. Rayleigh Disk 瑞利盘','3. Ring on the Rod 棒上环',
  '4. Unsinkable Disk 永不沉没的圆盘','5. Bimetallic Oscillator 双金属振荡器','6. Tennis Ball Tower 网球塔',
  '7. Three-Sided Dice 圆柱形骰子','8. Equipotential Lines 等势线','9. Water Spiral 水螺旋','10. Droplet Explosion 液滴爆炸','11. Balls on an Elastic Band 橡皮筋上的球','12. Strange Motion 奇怪的运动','13. Candle Powered Turbine 蜡烛动力涡旋机','14. Ball on Membrane 膜上球','15. Boycott Effect 抵制作用','16. Saving Honey 拯救蜂蜜','17. Invisibility 隐形'],
  ...
  onLoad: function (options) {
    ...
    vc.get().then(res=>{
      ...
      console.log('当前获取结果: '+res.data[3])
      console.log('当前选题限制: '+this.data.cf)
    })
    usf.get().then(res=>{
  ...
  selectBtnClick: function(){
    ...
    if(this.data.canse)
      console.log('当前可以选题: '+this.data.canse)
    else
      console.log('当前不能选题'+this.data.canse)
    ...
    usf.where({
      ...
      if(this.data.iscaptain){
        ...
        //         }
        //       })
        //       if (vc.data[4].tap[1]>0) {
        //         console.log('更新成功')
        //       }
        //       else  console.log('更新失败')
        //     }
        // else if(this.data.curpid==2)
        //     {
        ...
        //     }
          ...
          console.log("当前题号："+this.data.curpid+"，选题个数："+this.data.num[this.data.curpid]+"，选题限制："+this.data.cf)
